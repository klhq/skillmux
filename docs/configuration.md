...
- `[targets.<name>]`: one entry per adopted surface. `skillmux init --client <name> --yes` writes these for each supported client and scopes newly added targets to the current hostname; `skillmux target add <name> --dir <dir> --yes` writes one directly for a directory not tied to any supported client. Hand-editing is fine as long as `sync` is still allowed to own the directory (see below). An optional `host` limits the target to an exact machine-hostname match; omit it for a global, backward-compatible target. A host mismatch is reported and skipped before any target filesystem operation. `project_groups` is an explicit list, not a boolean: a target only receives the specific groups it names, never every group in the manifest.
...
`skillmux init --client <name> --yes` (or `skillmux target add <name> --dir
<dir> --yes` for a directory not tied to any supported client) first, which
