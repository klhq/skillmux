...
- Retrieval is hybrid: SQLite FTS5 (BM25) ∪ Harrier embedding cosine, reranked by a cross-encoder. Both models run on a remote model host: `microsoft/harrier-oss-v1-0.6b` (embeddings, 1024-dim) and `BAAI/bge-reranker-v2-m3` (reranker).
...
- Clients for Harrier embeddings and bge-reranker, with timeout → degraded fallback.
...
- Deployment: registration as an MCP gateway client, example Goose recipe configuration block and agent config snippets.
...
- Any gateway client split/permissions mapping — related but separate work.
- Serving the embedding/reranker models (already running on the remote host).
- Delivering supporting files' *contents* inline (AC5 lists their paths only; callers with filesystem access read them directly).
- Skill authoring, mutation, or promotion workflows.
- Multi-user auth (gateway layer owns access control).
...
- Both models are served by **Infinity** on the remote host. Embeddings are called **through the gateway proxy** (OpenAI-compatible `/v1/embeddings`). The reranker is called **directly at the remote Infinity endpoint** (or through a gateway passthrough) — Infinity's native `/rerank` API.
...
4. Embed + rerank clients with timeout fallback; hybrid union + config-driven threshold decision. (AC6, AC2 full path)
...
8. Deployment slice: Gateway MCP client registration + configuration examples; verify end-to-end from a recipe. (deployment scope)
